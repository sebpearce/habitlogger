...
module.exports = {
  ...
  getDoingsTable (next) {
    ...
    const sql = 'SELECT doings.id, doings.date, habits.name AS habit, doings.value, habits.type AS type FROM doings JOIN habits ON doings.habit = habits.id ORDER BY doings.date, doings.id';
    client.query(sql, function (err, result) {
      if (err) {
        console.error(err);
        res.send('Error ' + err);
